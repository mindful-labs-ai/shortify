...
- **영상 파이프라인 출처**: [mindful-labs-ai/video-cli](https://github.com/mindful-labs-ai/video-cli)의 9:16 컴포즈 스택을 Python 사이드카로 포팅하되, **모든 외부 AI 호출은 Google Gemini API로 통합** (Gemini 2.5 + Imagen + Veo + Gemini TTS + Gemini Audio)
...
| L7 External | 외부 AI/미디어 API | Google Gemini API (Gemini 2.5 + Imagen + Veo + Gemini TTS + Gemini Audio) |
...
[3] 목차 체크리스트 (최대 5개 소단위 선택)
        ▼
[4] 백그라운드: 섹션 텍스트 추출 + Gemini 2.5 Pro 4비트 추출
    UI: 5종 이미지 컨셉 카드 노출 → 사용자 선택
        ▼
[5] Imagen(이미지) → Veo(I2V) → Gemini TTS(나레이션)
    → Gemini Audio 정렬 → 리듬 컷 → ffmpeg 컴포즈
        ▼
...
| 2 | `conceptizing` | Gemini 2.5 Pro로 제목/주제/4비트 추출 | ~15s |
| 3 | `awaiting_image_choice` | 사용자 이미지 컨셉 선택 대기 | (사용자) |
| 4 | `generating_images` | Imagen 4로 14장 이미지 | ~60s |
| 5 | `generating_clips` | Veo 3 I2V로 14 클립 | ~5~7m |
| 6 | `generating_narration` | Gemini TTS (Gemini 2.5 Flash 네이티브 오디오) | ~30s |
| 7 | `aligning` | Gemini 2.5 Flash Audio (word-level timestamps) | ~30s |
...
| 이미지 생성 | Imagen 4 (Gemini API) |
| 영상 생성 (I2V) | Veo 3 / Veo 3 Fast (Gemini API) |
| 음성 합성 (TTS) | Gemini 2.5 Flash 네이티브 오디오 |
| 음성 정렬 | Gemini 2.5 Flash Audio (word-level timestamps) |
...
| 키 | 용도 |
|----|------|
| `GEMINI_API_KEY` | 모든 AI 호출 (Gemini 2.5 텍스트 + Imagen + Veo + Gemini TTS + Gemini Audio 정렬) |
...
| 항목 | 모델 | 단가 | 1편 사용량 | 소계 |
|------|------|------|------------|------|
| 컨셉 추출 + 씬 디렉션 | Gemini 2.5 Pro | $1.25/M in · $5/M out | ~10K in / 5K out | ~$0.04 |
| 이미지 (14장) | Imagen 4 | ~$0.04/장 | 14장 | ~$0.56 |
| I2V (14 클립 × 5초) | Veo 3 Fast | ~$0.15/sec | 70초 | ~$10.50 |
| 나레이션 TTS | Gemini 2.5 Flash 네이티브 오디오 | ~$0.50/M chars | ~1500 chars | ~$0.01 |
| 오디오 정렬 | Gemini 2.5 Flash Audio | 표준 audio 토큰 단가 | ~30초 | ~$0.01 |

**합계: ~$11/편** (Veo 3 Fast 기준). Veo 3 standard 사용 시 ~$28/편.
